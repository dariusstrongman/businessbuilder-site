...
export async function startBuild(_prev: StartState, form: FormData): Promise<StartState> {
  ...
  const echo = { idea, name, email, packageId: packageIds.has(pkg as PackageId) ? (pkg as PackageId) : undefined };
  ...
  return { status: "received", idea, packageId: echo.packageId, name, email };
}
