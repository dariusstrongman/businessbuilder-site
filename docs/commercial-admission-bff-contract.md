...
`npm run integration` uses deterministic backend responses to inspect rendered fixed-price, quote-required, delayed admission, evidence-review, and Verification states at 375/768/1440 with overflow and WCAG checks. It does **not** prove a real Stripe test-account transaction; that requires rotated credentials in the approved secret store, an isolated Business Builder Stripe test account, and verified test-mode webhook delivery. No live-charge flag or provider adapter is enabled here.
